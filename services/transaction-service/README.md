...
The Transaction Service is a critical component of the Piggy Bank system, responsible for handling all transaction operations including internal transfers, external bank transfers, and transaction status management.

## Overview

This microservice handles the complete lifecycle of financial transactions within Piggy Bank and between Piggy Bank and external banking systems. It maintains a reliable record of all transaction states and communicates with other services via Kafka events.

## Features

- Internal money transfers between Piggy Bank accounts
...
|--------|----------|-------------|----------------|
| POST | `/api/v1/transactions/transfer` | Initiate internal transfer between Piggy Bank accounts | JWT Required |
| POST | `/api/v1/transactions/transfer/external/outbound` | Initiate transfer from Piggy Bank to external bank | JWT Required |
