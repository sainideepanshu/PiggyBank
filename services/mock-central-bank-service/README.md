...
A simulated central bank service that facilitates interbank transactions between Piggy Bank and external financial institutions. This service is built as part of the Piggy Bank demo project to showcase external bank transfers.
...
- **Inbound Transfers API**: Processes transfers from Piggy Bank to external banks
- **Outbound Transfers API**: Processes transfers from external banks to Piggy Bank
...
### Inbound Transfers (Piggy Bank to External)
```
POST /api/v1/transfers/inbound
```
Used by Piggy Bank to initiate transfers to external banks.
...
### Outbound Transfers (External to Piggy Bank)
```
POST /api/v1/transfers/outbound
```
Used by external banks to send money to Piggy Bank accounts.
...
```
Used by Piggy Bank to report on the status of transfers they've processed.
